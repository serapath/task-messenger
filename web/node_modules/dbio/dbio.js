;(function dat_container_runtime () { // [initial version: 2024.05.18]
  ...
  async function container (REGISTRY, PROGRAMS) { // v2024.05.30
    ...
    if ('' in command) {
      ...
      console.log({fragment}, location)
      console.log(name, command) // name = '(sudo)', command = { demo: ... } // because #web_loader
  ...
  function web_loader () { // v2024.05.23
    ...
    const fragment = cmd_codec.encode(command)
  ...
  async function BOOTLOADER (port0, name, opts, spawn, box, bundle, program, loader, decode, encode, iframer) {  // v2024.05.30
    const cmd_codec = { encode, decode }
    console.log('%c[BOOTLOADER]', 'color:green;', 'load', loader.name)
    ...
    const loader_sdk = await (('' in opts) ? make_vault_sdk(window) : make_sys_sdk(window))
    ...
    console.log('%c[BOOTLOADER]', 'color:green;', 'load', name, opts)
    ...
    function prog_receive (event) {
      console.log('%c[BOOTLOADER]', 'color:green;', `[from ${program}]`, event)
    ...
    async function make_sys_sdk (window) { // -> sandboxed noscript iframe
      const el = iframer('allow-same-origin', iwindow => {
        iwindow.name = program // e.g. 'taskchat'
