;(function dat_container_runtime () { // [initial version: 2024.05.18]
  ...
  async function dbio_loader (REGISTRY, PROGRAMS) { // v2024.05.27
    ...
    function oncommand (event) {
      ...
      function onmessage ({ data: msg, ports: [port] }) {
        const {type, data} = msg
        on[type](data)
      ...
      function get (key){
        tasks[0].postMessage({data: localStorage[key], by: key})
      }
  ...
  async function BOOTLOADER (port0, name, opts, spawn, box, bundle, program, loader, decode, encode, iframer) {  // v2024.05.30
    ...
    const seed = result || `${Math.random()}`.slice(2) + `${Math.random()}`.slice(2)
    result || port0.postMessage(({type: 'set', data: {key: name, value: seed}}))
    ...
    **************************************************************************/
    ...
    async function get(name){
      port0.postMessage({type: 'get', data: name})
    ...
    function root_receive (event) {
      console.log('%c[BOOTLOADER]', 'color:green;', '[from dbio]', event.data)
      const message = event.data
      const { type = '', data = [], by = '' } = message || {}
      if(by)
        rx(event)
      else
        port1.postMessage(message)
      if (!by && app_api[type]) app_api(...data)
    }
    ...
    /**************************************************************************
      IO.js
    **************************************************************************/
    async function IO_js () { // IO.js
      const result = await get('io')
      const taken = result ? JSON.parse(result) : {}
      result || port0.postMessage(({type: 'set', data: {key: 'io', value: taken}}))
