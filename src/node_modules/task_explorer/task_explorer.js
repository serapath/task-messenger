...
function task_explorer (opts, protocol) {
  ...
  ******************************************/
  ...
  function add_node_sub ({ data, last, super_last, space, classes }) {
    ...
    element.innerHTML = `
      <div class="task_name">
        ${space}${last ? '└' : '├'}<span class="hub">📪</span><span class="tas">─📪</span><span class="inp">🗃</span><span class="out">─🗃</span><span class="name">${data.name}</span><span class="last">...</span>
      </div>
    `
    ...
    sub.onclick = () => {
      if(sub_open){
        ...
        hub_open ? sub.innerHTML = '┴📪' : sub.innerHTML = '─📪'
      } else{
        styles.innerHTML += `
        .link.output${data.id}{
          display: none;
