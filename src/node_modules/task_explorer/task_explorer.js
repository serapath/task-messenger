...
function task_explorer (opts, protocol) {
  ...
  function add_node_root ({ data, last }) {
    ...
    sub.onclick = () => {
      ...
      sub_open = handle_click({ el: tasks, type: 'tasks', data: data.tasks, space, check: sub_open, super_last: last, pos: false })
    }
  ...
  function add_node_sub ({ data, last, super_last, space }) {
    ...
    sup.onclick = () => {
      ...
      sup_open = handle_click({ el: sup_tasks, type: 'link', data: data.sup, space, check: sup_open, pos: true })
    }
    sub.onclick = () => {
      if(sub_open){
    ...
    inp.onclick = () => {
      ...
      inp_open = handle_click({ el: inputs, type: 'io', data: data.inputs, space, check: inp_open, pos: true })
    }
    out.onclick = () => {
      if(out_open){
      ...
      out_open = handle_click({ el: outputs, type: 'io', data: data.outputs, space, check: out_open, pos: false })
    }
    element.onfocus = node_onfocus
  ...
  function add_node_io ({ data, first, last, space }) {
    ...
    element.innerHTML = `
    <div class="task_name">
      <span class="space">${space}</span><span class="space_sup">${space_sup}</span>${first ? '┌' : last ? '└' : '├'}</span><span class="btn">📥─</span>${data.name}<span class="after">🔗</span>
    </div>
    <div class="tasks nodes">
    </div>
    `
    ...
    const tasks = element.querySelector('.tasks')
    btn.onclick = () => handle_click({ el: tasks, type: 'link', data: data.tasks, space, pos: false })
    return element
  }
  ...
  function handle_click ({ el, type, data, space, check, super_last, pos }) {
    el.classList.toggle('show')
    if(data && el.children.length < 1){
      length = data.length - 1
      data.forEach((value, i) => el.append(on_add[type]({ data: json_data[value], first: pos ? 0 === i : false, last: pos ? false : length === i, space, super_last })))
    }
    return !check
  }
